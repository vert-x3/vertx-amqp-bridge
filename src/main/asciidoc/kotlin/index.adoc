...
----
compile io.vertx:vertx-amqp-bridge:3.4.0-SNAPSHOT
----
...
bridge.start("localhost", 5672, { res ->
  ...
  var producer = bridge.createProducer<Any>("myAmqpAddress")

  var amqpMsgPayload = json {
    obj()
  }
...
bridge.start("localhost", 5672, { res ->
  ...
  var consumer = bridge.createConsumer<Any>("myAmqpAddress")
  consumer.handler({ vertxMsg ->
    var amqpMsgPayload = vertxMsg.body()
    var amqpBody = amqpMsgPayload.get<Any?>("body")
...
[source,kotlin]
----
var applicationProperties = json {
  obj()
}
...
var appProps = amqpMsgPayload.getJsonObject("application_properties")
if (appProps != null) {
  var propValue = appProps.get<Any?>("propertyName")
}
...
producer.send<JsonObject>(amqpMsgPayload, { res ->
  var amqpReplyMessagePayload = res.result().body()
...
consumer.handler({ msg ->
  ...
  if (replyAddress != null) {
    var amqpReplyMessagePayload = json {
      obj()
    }
